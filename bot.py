...
def reminder_text(practice: str) -> str:
    mapping = {
        "breath": "🌬 Напоминаю: можно сделать 3–5 спокойных циклов дыхания.\nИногда этого достаточно, чтобы телу стало чуть легче.",
        "sound_forest": "🎧 Напоминаю: можно включить звук леса и просто немного побыть в тишине с собой.",
        "ground": "🌳 Напоминаю: можно сделать короткое заземление и вернуть внимание в сегодняшний день.",
        "plan_water": "💧 Напоминаю: можно выпить воды или умыться — это простой способ вернуть телу ощущение опоры.",
...
def kb_after_step():
    ...
    kb.button(text="⏰ Напомнить мне по практике", callback_data="remind:choose")
    kb.button(text="🏠 В начало", callback_data="menu")
    kb.adjust(1, 1, 1)
    return kb.as_markup()
