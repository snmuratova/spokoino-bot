...
async def cb_breath(cb: CallbackQuery):
    ...
    text = (
        f"{header}\n\n"
        "Когда тревожность повышается, телу нужен короткий, понятный сигнал безопасности.\n\n"
...
async def q1(message: Message, state: FSMContext):
    ...
    if len(txt) < 2:
        await say(message, "Можно одной короткой фразой — что сейчас тревожит больше всего?", delay=0.05)
        return
...
async def cb_plan_choice(cb: CallbackQuery):
    ...
    if key == "water":
        ...
        msg = (
            ...
            "«Мне сейчас тревожно, можем 2 минуты поговорить?»\n\n"
            f"{praise(uid)}"
        )
    elif key == "facts":
        msg = (
